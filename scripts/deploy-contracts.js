...
async function main() {
  ...
  } else if (gasPrice && !gasLimit) options = { gasPrice: gasPrice }
  else if (gasLimit) {
    options = { gasLimit }
  } else options = null
  ...
  if (shouldDeployOceanToken || addresses.Ocean === null) {
    ...
    let ocean
    if (options)
      ocean = await Ocean.connect(owner).deploy(owner.address, options)
    ...
    ownershiptx = await ocean
      .connect(owner)
      .mint(owner.address, ethers.utils.parseUnits('1000000', 'ether'), options)
    ...
    if (process.env.MNEMONIC && networkDetails.chainId == 8996) {
      ...
      for (let walletNo = 1; walletNo < 10; walletNo++) {
        const oceanHolder = new Wallet.fromMnemonic(
          process.env.MNEMONIC,
          `m/44'/60'/0'/0/${walletNo}`
        )
        console.log('\t\tSending 100000 Ocean to ' + oceanHolder.address)
        const mintTx = await ocean
          .connect(owner)
          .mint(
            oceanHolder.address,
    ...
    if (OPFOwner != owner.address) {
      ...
      if (options)
        ownershiptx = await ocean
          .connect(owner)
          .transferOwnership(OPFOwner, options)
      else ownershiptx = await ocean.connect(owner).transferOwnership(OPFOwner)
      await ownershiptx.wait()
      if (sleepAmount > 0) await sleep(sleepAmount)
    }
  } else {
    if (OceanTokenAddress) addresses.Ocean = OceanTokenAddress
    //console.log("Using already deployed "+addresses.Ocean+" for Ocean token")
    else console.log('Using already deployed  for Ocean token')
  }
  ...
  if (shouldDeployMocks) {
    ...
    let DAI
    if (options)
      DAI = await ERC20Mock.connect(owner).deploy('DAI', 'DAI', 18, options)
    ...
    let USDC
    if (options)
      USDC = await ERC20Mock.connect(owner).deploy('USDC', 'USDC', 6, options)
    else USDC = await ERC20Mock.connect(owner).deploy('USDC', 'USDC', 6)
    addresses.MockUSDC = USDC.address
  }

  if (
    shouldDeployOPFCommunityFeeCollector ||
    ...
    if (options)
      opfcommunityfeecollector = await OPFCommunityFeeCollector.connect(
        owner
      ).deploy(OPFOwner, OPFOwner, options)
    else
      opfcommunityfeecollector = await OPFCommunityFeeCollector.connect(
        owner
      ).deploy(OPFOwner, OPFOwner)
  ...
  if (shouldDeployV4) {
    ...
    */
    ...
    if (addresses.EnterpriseFeeCollector) {
      ...
      if (options)
        fixedPriceExchangeEnterprise =
          await FixedrateExchangeEnterprise.connect(owner).deploy(
            router.address,
    ...
    let templateERC20
    if (options)
      templateERC20 = await ERC20Template.connect(owner).deploy(options)
    ...
    if (options)
      templateERC20Enterprise = await ERC20TemplateEnterprise.connect(
        owner
      ).deploy(options)
    else
      templateERC20Enterprise = await ERC20TemplateEnterprise.connect(
    ...
    if (shouldDeployPredictoor) {
      ...
      if (options)
        templateERC20Template3 = await ERC203Template.connect(owner).deploy(
          options
        )
    ...
    if (shouldDeploySaphireTemplate) {
      ...
      if (options)
        templateERC20Template4 = await ERC20Template4.connect(owner).deploy(
          options
        )
    ...
    let templateERC721
    if (options)
      templateERC721 = await ERC721Template.connect(owner).deploy(options)
    ...
    let dispenser
    if (options)
      dispenser = await Dispenser.connect(owner).deploy(router.address, options)
    ...
    let nftCount
    if (options)
      nftCount = await factoryERC721.getCurrentNFTTemplateCount(options)
    else nftCount = await factoryERC721.getCurrentNFTTemplateCount()
    if (sleepAmount > 0) await sleep(sleepAmount)
    let nftTemplate
    if (options)
      nftTemplate = await factoryERC721.getNFTTemplate(nftCount, options)
    ...
    let tokenTemplate
    if (options)
      tokenTemplate = await factoryERC721.getTokenTemplate(
        currentTokenCount,
        options
      )
    ...
    let templateadd
    if (options)
      templateadd = await factoryERC721
        .connect(owner)
        .addTokenTemplate(templateERC20Enterprise.address, options)
    else
      templateadd = await factoryERC721
        .connect(owner)
        .addTokenTemplate(templateERC20Enterprise.address)
    ...
    if (shouldDeployPredictoor) {
      ...
      if (options)
        templateadd = await factoryERC721
          .connect(owner)
          .addTokenTemplate(templateERC20Template3.address, options)
      else
        templateadd = await factoryERC721
          .connect(owner)
          .addTokenTemplate(templateERC20Template3.address)
      await templateadd.wait()
      if (sleepAmount > 0) await sleep(sleepAmount)
      if (options)
        currentTokenCount = await factoryERC721.getCurrentTemplateCount(options)
    ...
    if (shouldDeploySaphireTemplate) {
      ...
      if (options)
        templateadd = await factoryERC721
          .connect(owner)
          .addTokenTemplate(templateERC20Template4.address, options)
      else
        templateadd = await factoryERC721
          .connect(owner)
          .addTokenTemplate(templateERC20Template4.address)
      await templateadd.wait()
      if (sleepAmount > 0) await sleep(sleepAmount)
      if (options)
        currentTokenCount = await factoryERC721.getCurrentTemplateCount(options)
    ...
    if (logging)
      console.info(
        'Adding factoryERC721.address(' + factoryERC721.address + ') to router'
      )
    let factoryAddTx
    if (options)
      factoryAddTx = await router
        .connect(owner)
        .addFactory(factoryERC721.address, options)
    else
      factoryAddTx = await router
        .connect(owner)
        .addFactory(factoryERC721.address)
    await factoryAddTx.wait()
    if (sleepAmount > 0) await sleep(sleepAmount)

    if (logging)
      console.info(
        'Adding fixedPriceExchange.address(' +
          fixedPriceExchange.address +
          ') to router'
      )
    let freAddTx
    if (options)
      freAddTx = await router
        .connect(owner)
        .addFixedRateContract(fixedPriceExchange.address, options)
    else
      freAddTx = await router
        .connect(owner)
        .addFixedRateContract(fixedPriceExchange.address)
    await freAddTx.wait()
    if (sleepAmount > 0) await sleep(sleepAmount)
    ...
    if (addresses.FixedPriceEnterprise) {
      if (logging)
        console.info(
          'Adding FixedPriceEnterprise.address(' +
            addresses.FixedPriceEnterprise +
            ') to router'
        )
      let enterpriseFreAddTx
      if (options)
        enterpriseFreAddTx = await router
          .connect(owner)
          .addFixedRateContract(addresses.FixedPriceEnterprise, options)
      else
        enterpriseFreAddTx = await router
          .connect(owner)
          .addFixedRateContract(addresses.FixedPriceEnterprise)
    ...
    if (logging)
      console.info(
        'Adding dispenser.address(' + dispenser.address + ') to router'
      )
    let dispenserAddTx
    if (options)
      dispenserAddTx = await router
        .connect(owner)
        .addDispenserContract(dispenser.address, options)
    else
      dispenserAddTx = await router
        .connect(owner)
        .addDispenserContract(dispenser.address)
    ...
    for (const token of additionalApprovedTokens) {
      ...
      let tokenTx
      if (options)
        tokenTx = await router.connect(owner).addApprovedToken(token, options)
    ...
    if (owner.address != routerOwner) {
      ...
      let routerOwnerTx
      if (options)
        routerOwnerTx = await router
          .connect(owner)
          .changeRouterOwner(routerOwner, options)
      else
        routerOwnerTx = await router
          .connect(owner)
          .changeRouterOwner(routerOwner)
  ...
  if (shouldDeployVE) {
    ...
    const veOCEAN = await ethers.getContractFactory('veOCEAN', owner)
    const deployedVEOCEAN = await veOCEAN
      .connect(owner)
      .deploy(addresses.Ocean, 'veOCEAN', 'veOCEAN', '0.1.0', options)
    ...
    const veAllocate = await ethers.getContractFactory('veAllocate', owner)
    ...
    const veDelegation = await ethers.getContractFactory('veDelegation', owner)
    const deployedVEDelegation = await veDelegation
      .connect(owner)
      .deploy(
    ...
    const deployedFeeDistributor = await FeeDistributor.connect(owner).deploy(
      addresses.veOCEAN,
      timestamp,
      addresses.Ocean,
      routerOwner,
    ...
    const veFeeEstimate = await ethers.getContractFactory(
      'veFeeEstimate',
      owner
    )
    const deployedVeFeeEstimate = await veFeeEstimate
      .connect(owner)
      .deploy(addresses.veOCEAN, addresses.veFeeDistributor, options)
    ...
    const SmartWalletChecker = await ethers.getContractFactory(
      'SmartWalletChecker',
      owner
    )
    const deployedSmartWalletChecker = await SmartWalletChecker.connect(
      owner
    ).deploy(options)
    await deployedSmartWalletChecker.deployTransaction.wait()
    addresses.SmartWalletChecker = deployedSmartWalletChecker.address
    const commit_checker = await deployedVEOCEAN
      .connect(owner)
      .commit_smart_wallet_checker(addresses.SmartWalletChecker, options)
    await commit_checker.wait()
    const apply_checker = await deployedVEOCEAN
      .connect(owner)
      .apply_smart_wallet_checker(options)
    ...
    if (routerOwner != owner.address) {
      ...
      let tx = await deployedVEOCEAN
        .connect(owner)
        .commit_transfer_ownership(routerOwner, options)
      await tx.wait()
      tx = await deployedVEOCEAN
        .connect(owner)
        .apply_transfer_ownership(options)
      await tx.wait()
      tx = await deployedSmartWalletChecker
        .connect(owner)
        .setManager(routerOwner, true, options)
      await tx.wait()
      tx = await deployedSmartWalletChecker
        .connect(owner)
        .setManager(owner.address, false, options)
      await tx.wait()
    }
  }
  ...
  if (shouldDeployVesting) {
    ...
    const Splitter = await ethers.getContractFactory('Splitter', owner)
    const deploySplitter = await Splitter.connect(owner).deploy(
      [OPFOwner],
      [100],
      options
    )
    ...
    if (show_verify) {
      ...
      console.log(
        '\tnpx hardhat verify --network ' +
          networkName +
          ' --constructor-args splitter-args.js ' +
          addresses.Splitter
      )
    }
    if (sleepAmount > 0) await sleep(sleepAmount)
    ...
    const deployVestingWallet0 = await VestingWallet0.connect(owner).deploy(
      addresses.Splitter,
      blockTimestamp,
      vestingPeriod,
      options
    ...
    const Distribute = await ethers.getContractFactory('Distribute', owner)
    ...
    if (routerOwner != owner.address) {
      ...
      let tx = await deploySplitter
        .connect(owner)
        .transferOwnership(routerOwner, options)
      await tx.wait()
      tx = await deployVestingWallet0
        .connect(owner)
        .transferOwnership(routerOwner, options)
      await tx.wait()
    }
  ...
  const BatchPayments = await ethers.getContractFactory('BatchPayments', owner)
  ...
  if (addresses.EnterpriseFeeCollector) {
    ...
    const deployEnterpriseEscrow = await EnterpriseEscrow.connect(owner).deploy(
      addresses.EnterpriseFeeCollector,
      options
    )
    ...
    if (show_verify) {
      ...
      console.log(
        '\tnpx hardhat verify --network ' +
          networkName +
          ' ' +
          deployEnterpriseEscrow.address +
          ' ' +
          addresses.EnterpriseFeeCollector
      )
    }
  ...
  if (shouldDeployDF) {
    ...
    const DFRewards = await ethers.getContractFactory('DFRewards', owner)
    ...
    const DFStrategyV1 = await ethers.getContractFactory('DFStrategyV1', owner)
    const deployedDFStrategyV1 = await DFStrategyV1.connect(owner).deploy(
      addresses.DFRewards,
      options
    )
    ...
    const strategyTx = await deployedDFRewards
      .connect(owner)
      .addStrategy(addresses.DFStrategyV1, options)
    await strategyTx.wait()
    if (owner.address != routerOwner) {
      if (logging) console.info('Moving ownerships to ' + routerOwner)
      const DFRewardsOwnerTx = await deployedDFRewards
        .connect(owner)
        .transferOwnership(routerOwner, options)
  ...
  if (shouldDeployPredictoorHelper == true) {
    ...
    const PredictoorHelper = await ethers.getContractFactory(
      'PredictoorHelper',
      owner
    )
    const deployedPredictoorHelper = await PredictoorHelper.connect(
      owner
    ).deploy()
    ...
    if (pdrTrueValSubmiter && pdrTrueValSubmiter != owner.address) {
      ...
      if (options)
        ownershiptx = await deployedPredictoorHelper
          .connect(owner)
          .transferOwnership(pdrTrueValSubmiter, options)
      else
        ownershiptx = await deployedPredictoorHelper
          .connect(owner)
          .transferOwnership(pdrTrueValSubmiter)
      await ownershiptx.wait()
      if (sleepAmount > 0) await sleep(sleepAmount)
    }
  }
  ...
  const accessListFactoryContract = await AccessListFactory.connect(
    owner
  ).deploy(accessListContract.address)
  ...
  if (owner.address != routerOwner) {
    if (logging) console.info('Moving ownerships to ' + routerOwner)
    const accessListFactoryContractOwnerTx = await accessListFactoryContract
      .connect(owner)
      .transferOwnership(routerOwner, options)
  ...
  await votingWeightContract.deployTransaction.wait()
  addresses.VotingWeight = votingWeightContract.address
  if (show_verify) {
    console.log('\tRun the following to verify on etherscan')
    console.log(
      '\tnpx hardhat verify --network ' +
        networkName +
        ' ' +
        addresses.VotingWeight
    )
  }

  // MetadataRequestManager
  if (logging) console.info('Deploying MetadataRequestManager')
  const MetadataRequestManager = await ethers.getContractFactory(
    'MetadataRequestManager'
  )
