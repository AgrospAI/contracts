...
async function main() {
  ...
  if (shouldDeployV4) {
    ...
    */
    ...
    let dispenser;
    if (options)
      dispenser = await Dispenser.connect(owner).deploy(
        router.address,
        options
      );
  ...
  const deployBatchPayments = await BatchPayments.connect(owner).deploy(
    options
  );
  ...
  const metadataRequestManagerContract = await MetadataRequestManager.connect(owner).deploy()
